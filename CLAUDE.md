...
- Source: `cli_anything/espresense/`
  ...
  - `tests/` — 1509 tests, synthetic data, no live services
...
python3 -m pytest cli_anything/espresense/tests/ -v       # test (1509 tests)
...
- `core/telemetry.py` is the *query* half of the harness (v0.3.0): where a device was last seen (`devices whereis`, exits 1 with `found: false` when never seen — same gating shape as `rooms locate`), who is in which room (`devices occupancy`), which nodes hear which devices at what distance (`mqtt distances`, aggregated over `<prefix>/rooms/+/devices/+`), and which nodes are online (`mqtt node-status`, retained `<prefix>/rooms/<node>/status`). v0.7.0 added the health half: `mqtt telemetry` aggregates `<prefix>/rooms/<node>/telemetry` per node — sample count, worst-case readings over the window (max `uptime`, min `free_mem`, min `rssi`) and the most recent payload in `latest`, with `lowest_free_mem` flagging the tightest node. `parse_telemetry_payload` normalises firmware key spellings (`freeMem`→`free_mem`, `ver`→`version`), passes unknown keys through so a newer firmware's extra fields stay visible, and rejects anything that is not a JSON object (or has a non-numeric string in a numeric field) so one malformed publisher cannot corrupt a snapshot. All aggregation is pure over collected `mqtt.watch` records; `parse_distance_payload` accepts both the bare-number and `{"distance": ...}` payload shapes nodes have shipped, and drops anything else rather than corrupting a snapshot.
- `core/history.py` gained the pure `trail(rows)` fold (v0.6.0): `history trail DEVICE_ID` turns the raw `/api/history/<id>` rows into consecutive room segments (one per visit, not one per room), first/last-seen timestamps and the ordered `rooms_visited`. It accepts the same alternate row spellings as `telemetry.whereis` (`roomName`/`room`, `unixTs`/`ts`/`timestamp`); rows without a room stay as `room: None` segments so point counts stay honest but don't count as visits. Unlike `whereis`, `trail` exits 0 on an empty history — it summarises whatever rows exist.
- `history heatmap` (v0.8.0) is the fleet-level counterpart: pure `heatmap(history_by_device)` folds many devices' rows (via `trail`) into one per-room usage record — points, visits, dwell `seconds` (sum of visit spans, a lower bound; non-numeric timestamps contribute 0) and the devices that visited, sorted most-used first with the `room: None` bucket pinned last and never counted as visits. The CLI command does the transport: `--device` (repeatable) restricts and skips the `/api/state/devices` discovery call; otherwise every tracked device id is fetched. Empty per-device histories are dropped from the mapping but still counted in `devices_queried`.
